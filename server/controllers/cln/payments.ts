...
function groupBy(payments) {
  ...
  const paymentsGrpArray = Object.keys(paymentsInGroups).map((key) =>
    paymentsInGroups[key].length && paymentsInGroups[key].length > 1
        ...
        if (req.body.saveToDB && req.body.bolt12) {
          ...
          return databaseService
            .update(
              ...
              CollectionsEnum.OFFERS,
              offerToUpdate,
              CollectionFieldsEnum.BOLT12,
              req.body.bolt12
            )
            .then((updatedOffer) => {
