...
export const getInfo = (req, res, next) => {
  logger.log({
    ...
    level: 'INFO',
    fileName: 'GetInfo',
    msg: 'Getting Core Lightning Node Information..'
  });
  ...
  logger.log({
    ...
    level: 'INFO',
    fileName: 'GetInfo',
    msg: 'Calling Info from Core Lightning server url ' + options.url
  });
  if (!options.headers || !options.headers.macaroon) {
    ...
    const err = common.handleError(
      { statusCode: 502, message: 'Bad Macaroon', error: errMsg },
      'GetInfo',
      errMsg,
