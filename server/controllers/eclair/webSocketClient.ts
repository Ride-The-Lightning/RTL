...
export class ECLWebSocketClient {
  ...
  public reconnet = (eclWsClt) => {
    ...
    this.reconnectTimeOut = setTimeout(() => {
      if (eclWsClt.selectedNode) {
        this.logger.log({
          selectedNode: eclWsClt.selectedNode,
          level: 'INFO',
          fileName: 'ECLWebSocket',
          msg: "Reconnecting to the Eclair's Websocket Server.."
        });
  ...
  public connect = (selectedNode: CommonSelectedNode) => {
    try {
      ...
      if (!clientExists) {
        ...
        if (
          (!clientExists.webSocketClient || clientExists.webSocketClient.readyState !== WebSocket.OPEN) &&
  ...
  public connectWithClient = (eclWsClt) => {
    ...
    const WS_LINK =
      UpdatedLNServerURL.slice(0, firstSubStrIndex) +
      ':' +
      eclWsClt.selectedNode.ln_api_password +
      '@' +
      UpdatedLNServerURL.slice(firstSubStrIndex) +
      '/ws';
    ...
    eclWsClt.webSocketClient.onopen = () => {
      this.logger.log({
        ...
        level: 'INFO',
        fileName: 'ECLWebSocket',
        msg: "Connected to the Eclair's Websocket Server.."
      });
      this.waitTime = 0.5;
    };
    ...
    eclWsClt.webSocketClient.onerror = (err) => {
      if (
        eclWsClt.selectedNode.ln_version === '' ||
        !eclWsClt.selectedNode.ln_version ||
        this.common.isVersionCompatible(eclWsClt.selectedNode.ln, '0.5.0')
      ) {
        this.logger.log({
          selectedNode: eclWsClt.selectedNode,
          level: 'ERROR',
  ...
  public disconnect = (selectedNode: CommonSelectedNode) => {
    ...
    if (clientExists && clientExists.webSocketClient && clientExists.webSocketClient.readyState === WebSocket.OPEN) {
      this.logger.log({
        ...
        level: 'INFO',
        fileName: 'ECLWebSocket',
        msg: "Disconnecting from the Eclair's Websocket Server.."
      });
