...
export const getFilteredNodes = (selNode: CommonSelectedNode, peersNodeIds) => {
  ...
  return request.post(options).then((nodes) => {
    logger.log({ selectedNode: selNode, level: 'INFO', fileName: 'Peers', msg: 'Filtered Nodes', data: nodes });
    return nodes;
  }).catch((err) => []);
};
...
export const connectPeer = (req, res, next) => {
  ...
  if (req.query) {
    ...
    logger.log({ selectedNode: req.session.selectedNode, level: 'INFO', fileName: 'Peers', msg: 'Connect Peer Params', data: options.form });
  }
  request.post(options).then((body) => {
    logger.log({ selectedNode: req.session.selectedNode, level: 'INFO', fileName: 'Peers', msg: 'Added Peer', data: body });
    ...
    request.post(options).then((body) => {
      ...
      if (body && body.length) {
        ...
        return getFilteredNodes(req.session.selectedNode, peersNodeIds).then((peersWithAlias) => {
          ...
          logger.log({ selectedNode: req.session.selectedNode, level: 'INFO', fileName: 'Peers', msg: 'Peers List with Newest On Top', data: peers });
...
export const deletePeer = (req, res, next) => {
  ...
  if (req.params.nodeId) {
    ...
    logger.log({ selectedNode: req.session.selectedNode, level: 'INFO', fileName: 'Peers', msg: 'Disconnect Peer Params', data: options.form });
  }
  request.post(options).then((body) => {
