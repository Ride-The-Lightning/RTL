...
export const getPeers = (req, res, next) => {
  ...
  if (common.read_dummy_data) {
    common.getDummyData('Peers', req.session.selectedNode.ln_implementation).then((data) => {
    ...
    request
      ...
      .then((body) => {
        logger.log({
          ...
          level: 'DEBUG',
          fileName: 'Peers',
          msg: 'Peers List Received',
          data: body
        });
        if (body && body.length) {
          let peersNodeIds = '';
          body.forEach((peer) => {
            peersNodeIds = peersNodeIds + ',' + peer.nodeId;
          });
          ...
          return getFilteredNodes(req.session.selectedNode, peersNodeIds).then((peersWithAlias) => {
            ...
            logger.log({
              ...
              level: 'INFO',
              fileName: 'Peers',
              msg: 'Sorted Peers List Received',
              data: body
            });
    ...
    .then((body) => {
      ...
      if (typeof body === 'string' && body.includes('already connected')) {
        const err = common.handleError(
          { statusCode: 500, message: 'Connect Peer Error', error: body },
          'Peers',
          body,
        ...
        .then((body) => {
          ...
          if (body && body.length) {
            ...
            return getFilteredNodes(req.session.selectedNode, peersNodeIds).then((peersWithAlias) => {
              ...
              peers = common.newestOnTop(
                peers,
                'nodeId',
                req.query.nodeId
                  ? req.query.nodeId
                  : req.query.uri
                  ? req.query.uri.substring(0, req.query.uri.indexOf('@'))
                  : ''
              );
              logger.log({
                selectedNode: req.session.selectedNode,
                level: 'INFO',
                fileName: 'Peers',
                msg: 'Peers List after Connect Received',
                data: peers
              });
