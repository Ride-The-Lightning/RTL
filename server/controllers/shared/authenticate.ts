...
const handleMultipleFailedAttemptsError = (failed, currentTime, errMsg) => {
  if (failed.count >= ALLOWED_LOGIN_ATTEMPTS && currentTime <= failed.lastTried + LOCKING_PERIOD) {
    return {
      ...
      error:
        'Application locked for ' +
        LOCKING_PERIOD / ONE_MINUTE +
        ' minutes due to multiple failed attempts!\nTry again after ' +
        common.convertTimestampToTime((failed.lastTried + LOCKING_PERIOD) / 1000) +
        '!'
    };
  } else {
    return {
      ...
      error:
        errMsg +
        '\nApplication will be locked after ' +
        (ALLOWED_LOGIN_ATTEMPTS - failed.count) +
...
export const verifyToken = (twoFAToken) =>
  !!(
    ...
    common.rtl_secret2fa !== '' &&
    otplib.authenticator.check(twoFAToken, common.rtl_secret2fa)
  );
...
export const authenticateUser = (req, res, next) => {
  ...
  if (+common.rtl_sso) {
    if (req.body.authenticateWith === 'JWT' && jwt.verify(req.body.authenticationValue, common.secret_key)) {
      ...
      res
        .status(406)
        .json({ message: 'SSO Authentication Error', error: 'Login with Password is not allowed with SSO.' });
    } else if (req.body.authenticateWith === 'PASSWORD') {
      if (
        common.cookie_value.trim().length >= 32 &&
        crypto.timingSafeEqual(
          Buffer.from(crypto.createHash('sha256').update(common.cookie_value).digest('hex'), 'utf-8'),
          Buffer.from(req.body.authenticationValue, 'utf-8')
        )
      ) {
        ...
        const err = common.handleError(
          { statusCode: 406, message: 'SSO Authentication Error', error: errMsg },
          'Authenticate',
          errMsg,
    ...
    if (common.rtl_pass === password && failed.count < ALLOWED_LOGIN_ATTEMPTS) {
      if (req.body.twoFAToken && req.body.twoFAToken !== '') {
        if (!verifyToken(req.body.twoFAToken)) {
          logger.log({
            ...
            level: 'ERROR',
            fileName: 'Authenticate',
            msg: 'Invalid Token! Failed IP ' + reqIP,
      ...
      logger.log({
        ...
        level: 'ERROR',
        fileName: 'Authenticate',
        msg: 'Invalid Password! Failed IP ' + reqIP,
...
export const resetPassword = (req, res, next) => {
  ...
  if (+common.rtl_sso) {
    ...
    const err = common.handleError(
      { statusCode: 401, message: 'Password Reset Error', error: errMsg },
      'Authenticate',
      errMsg,
      req.session.selectedNode
    );
