...
export const loopOut = (req, res, next) => {
  ...
  if (options.url === '') {
    ...
    const err = common.handleError(
      { statusCode: 500, message: 'Loop Out Error', error: errMsg },
      'Loop',
      errMsg,
...
export const loopOutTerms = (req, res, next) => {
  logger.log({
    ...
    level: 'INFO',
    fileName: 'Loop',
    msg: 'Getting Loop Out Terms..'
  });
  options = common.getSwapServerOptions(req);
  if (options.url === '') {
    const errMsg = 'Loop Server URL is missing in the configuration.';
    const err = common.handleError(
      { statusCode: 500, message: 'Loop Out Terms Error', error: errMsg },
      'Loop',
      errMsg,
      req.session.selectedNode
    );
...
export const loopOutQuote = (req, res, next) => {
  logger.log({
    ...
    level: 'INFO',
    fileName: 'Loop',
    msg: 'Getting Loop Out Quotes..'
  });
  options = common.getSwapServerOptions(req);
  if (options.url === '') {
    const errMsg = 'Loop Server URL is missing in the configuration.';
    const err = common.handleError(
      { statusCode: 500, message: 'Loop Out Quotes Error', error: errMsg },
      'Loop',
      errMsg,
      req.session.selectedNode
    );
  ...
  options.url =
    ...
    req.params.amount +
    '?conf_target=' +
    (req.query.targetConf ? req.query.targetConf : '2') +
    '&swap_publication_deadline=' +
    req.query.swapPublicationDeadline;
  logger.log({
    selectedNode: req.session.selectedNode,
    level: 'DEBUG',
    fileName: 'Loop',
    msg: 'Loop Out Quote URL',
    data: options.url
  });
  request(options)
    .then((quoteRes) => {
      quoteRes.amount = +req.params.amount;
      quoteRes.swap_payment_dest = quoteRes.swap_payment_dest
        ? Buffer.from(quoteRes.swap_payment_dest, 'base64').toString('hex')
        : '';
      logger.log({
        selectedNode: req.session.selectedNode,
        level: 'INFO',
        fileName: 'Loop',
        msg: 'Loop Out Quote Received',
        data: quoteRes
      });
...
export const loopOutTermsAndQuotes = (req, res, next) => {
  logger.log({
    ...
    level: 'INFO',
    fileName: 'Loop',
    msg: 'Getting Loop Out Terms & Quotes..'
  });
  options = common.getSwapServerOptions(req);
  if (options.url === '') {
    const errMsg = 'Loop Server URL is missing in the configuration.';
    const err = common.handleError(
      { statusCode: 500, message: 'Loop Out Terms & Quotes Error', error: errMsg },
      'Loop',
      errMsg,
      req.session.selectedNode
    );
  ...
  request(options)
    .then((terms) => {
      ...
      logger.log({
        ...
        level: 'DEBUG',
        fileName: 'Loop',
        msg: 'Loop Out Min Quote Options',
        data: options1
      });
      logger.log({
        selectedNode: req.session.selectedNode,
        level: 'DEBUG',
        fileName: 'Loop',
        msg: 'Loop Out Max Quote Options',
        data: options2
      });
      return Promise.all([request(options1), request(options2)])
        .then((values) => {
          ...
          values[0].swap_payment_dest = values[0].swap_payment_dest
            ? Buffer.from(values[0].swap_payment_dest, 'base64').toString('hex')
            : '';
          values[1].swap_payment_dest = values[1].swap_payment_dest
            ? Buffer.from(values[1].swap_payment_dest, 'base64').toString('hex')
            : '';
          logger.log({
            selectedNode: req.session.selectedNode,
            level: 'INFO',
            fileName: 'Loop',
            msg: 'Loop Out Quotes 1 Received',
            data: values[0]
          });
          logger.log({
            selectedNode: req.session.selectedNode,
            level: 'INFO',
            fileName: 'Loop',
            msg: 'Loop Out Quotes 2 Received',
            data: values[1]
          });
...
export const loopIn = (req, res, next) => {
  ...
  if (options.url === '') {
    const errMsg = 'Loop Server URL is missing in the configuration.';
    const err = common.handleError(
      { statusCode: 500, message: 'Loop In Error', error: errMsg },
      'Loop',
      errMsg,
      req.session.selectedNode
    );
...
export const loopInTerms = (req, res, next) => {
  logger.log({
    ...
    level: 'INFO',
    fileName: 'Loop',
    msg: 'Getting Loop In Terms..'
  });
  options = common.getSwapServerOptions(req);
  if (options.url === '') {
    const errMsg = 'Loop Server URL is missing in the configuration.';
    const err = common.handleError(
      { statusCode: 500, message: 'Loop In Terms Error', error: errMsg },
      'Loop',
      errMsg,
      req.session.selectedNode
    );
...
export const loopInQuote = (req, res, next) => {
  logger.log({
    ...
    level: 'INFO',
    fileName: 'Loop',
    msg: 'Getting Loop In Quotes..'
  });
  options = common.getSwapServerOptions(req);
  if (options.url === '') {
    const errMsg = 'Loop Server URL is missing in the configuration.';
    const err = common.handleError(
      { statusCode: 500, message: 'Loop In Quotes Error', error: errMsg },
      'Loop',
      errMsg,
      req.session.selectedNode
    );
  ...
  logger.log({
    ...
    level: 'DEBUG',
    fileName: 'Loop',
    msg: 'Loop In Quote Options',
    data: options.url
  });
  request(options)
    .then((body) => {
      body.amount = +req.params.amount;
      body.swap_payment_dest = body.swap_payment_dest
        ? Buffer.from(body.swap_payment_dest, 'base64').toString('hex')
        : '';
      logger.log({
        selectedNode: req.session.selectedNode,
        level: 'INFO',
        fileName: 'Loop',
        msg: 'Loop In Qoutes Received',
        data: body
      });
      res.status(200).json(body);
    })
...
export const loopInTermsAndQuotes = (req, res, next) => {
  logger.log({
    ...
    level: 'INFO',
    fileName: 'Loop',
    msg: 'Getting Loop In Terms & Quotes..'
  });
  options = common.getSwapServerOptions(req);
  if (options.url === '') {
    const errMsg = 'Loop Server URL is missing in the configuration.';
    const err = common.handleError(
      { statusCode: 500, message: 'Loop In Terms & Quotes Error', error: errMsg },
      'Loop',
      errMsg,
      req.session.selectedNode
    );
...
export const swaps = (req, res, next) => {
  ...
  if (options.url === '') {
    const errMsg = 'Loop Server URL is missing in the configuration.';
    const err = common.handleError(
      { statusCode: 500, message: 'List Swaps Error', error: errMsg },
      'Loop',
      errMsg,
      req.session.selectedNode
    );
    ...
    .then((body) => {
      ...
      if (body.swaps && body.swaps.length > 0) {
        ...
        logger.log({
          ...
          level: 'INFO',
          fileName: 'Loop',
          msg: 'Sorted Loop Swaps List Received',
          data: body
        });
      }
...
export const swap = (req, res, next) => {
  logger.log({
    ...
    level: 'INFO',
    fileName: 'Loop',
    msg: 'Getting Swap Information..'
  });
  options = common.getSwapServerOptions(req);
  if (options.url === '') {
    const errMsg = 'Loop Server URL is missing in the configuration.';
    const err = common.handleError(
      { statusCode: 500, message: 'Get Swap Error', error: errMsg },
      'Loop',
      errMsg,
      req.session.selectedNode
    );
  ...
  request(options)
    .then((body) => {
      logger.log({
        ...
        level: 'INFO',
        fileName: 'Loop',
        msg: 'Loop Swap Information Received',
        data: body
      });
      res.status(200).json(body);
    })
