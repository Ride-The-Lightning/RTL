    ...
    .then((body) => {
      ...
      return Promise.all(peers.map((peer) => getAliasForPeers(req.session.selectedNode, peer))).then((values) => {
        logger.log({
          ...
          level: 'DEBUG',
          fileName: 'Peers',
          msg: 'Peers with Alias before Sort',
          data: body
        });
        if (body.peers) {
  ...
  request
    ...
    .then((body) => {
      ...
      request(options)
        .then((body) => {
          const peers = !body.peers ? [] : body.peers;
          return Promise.all(peers.map((peer) => getAliasForPeers(req.session.selectedNode, peer)))
            .then((values) => {
              if (body.peers) {
                body.peers = common.sortDescByStrKey(body.peers, 'alias');
...
export const deletePeer = (req, res, next) => {
  ...
  logger.log({
    ...
    level: 'INFO',
    fileName: 'Peers',
    msg: 'Peer Disconnect Pubkey',
    data: req.params.peerPubKey
  });
  request
    .delete(options)
    .then((body) => {
