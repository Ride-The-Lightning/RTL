...
export const invoiceLookup = (req, res, next) => {
  logger.log({
    ...
    level: 'INFO',
    fileName: 'Invoice',
    msg: 'Getting Invoice Information..'
  });
  ...
  request(options)
    .then((body) => {
      ...
      body.description_hash = body.description_hash
        ? Buffer.from(body.description_hash, 'base64').toString('hex')
        : null;
      logger.log({
        selectedNode: req.session.selectedNode,
        level: 'INFO',
        fileName: 'Invoice',
        msg: 'Invoice Information Received',
        data: body
      });
    ...
    .then((body) => {
      ...
      if (body.invoices && body.invoices.length > 0) {
        body.invoices.forEach((invoice) => {
          ...
          invoice.description_hash = invoice.description_hash
            ? Buffer.from(invoice.description_hash, 'base64').toString('hex')
            : null;
        });
    ...
    .then((body) => {
      ...
      try {
        ...
        const err = common.handleError(
          errRes,
          'Invoices',
          'Subscribe to Newly Added Invoice Error',
        ...
        logger.log({
          ...
          level: 'ERROR',
          fileName: 'Invoice',
          msg: 'Subscribe to Newly Added Invoice Error',
          error: err
        });
      }
      body.r_preimage = body.r_preimage ? Buffer.from(body.r_preimage, 'base64').toString('hex') : '';
