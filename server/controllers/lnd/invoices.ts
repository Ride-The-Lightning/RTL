...
// TLV type for keysend message (34349334 is the standard keysend message record type)
const KEYSEND_MESSAGE_TLV_TYPE = '34349334';

const extractKeysendMessage = (invoice) => {
  // Only process keysend invoices that have HTLCs and no existing memo
  if (invoice.is_keysend && (!invoice.memo || invoice.memo === '') && invoice.htlcs && invoice.htlcs.length > 0) {
    for (const htlc of invoice.htlcs) {
      if (htlc.custom_records && htlc.custom_records[KEYSEND_MESSAGE_TLV_TYPE]) {
        try {
          invoice.memo = Buffer.from(htlc.custom_records[KEYSEND_MESSAGE_TLV_TYPE], 'base64').toString('utf8');
          break;
        } catch (err) {
          // If decoding fails, leave memo empty
        }
      }
    }
  }
