...
export class LNDWebSocketClient {
  ...
  public subscribeToInvoice = (options: any, selectedNode: CommonSelectedNode, rHash: string) => {
    ...
    this.logger.log({
      ...
      level: 'INFO',
      fileName: 'WebSocketClient',
      msg: 'Subscribing to Invoice ' + rHash + ' ..'
    });
    ...
    request(options)
      .then((msg) => {
        this.logger.log({
          ...
          level: 'INFO',
          fileName: 'WebSocketClient',
          msg: 'Invoice Information Received for ' + rHash
        });
        if (typeof msg === 'string') {
          const results = msg.split('\n');
          msg = results.length && results.length > 1 ? JSON.parse(results[1]) : JSON.parse(msg);
          msg.result.r_preimage = msg.result.r_preimage
            ? Buffer.from(msg.result.r_preimage, 'base64').toString('hex')
            : '';
          msg.result.r_hash = msg.result.r_hash ? Buffer.from(msg.result.r_hash, 'base64').toString('hex') : '';
          msg.result.description_hash = msg.result.description_hash
            ? Buffer.from(msg.result.description_hash, 'base64').toString('hex')
            : null;
        }
        msg['type'] = 'invoice';
        msg['source'] = 'LND';
        ...
        this.logger.log({
          ...
          level: 'INFO',
          fileName: 'WebSocketClient',
          msg: 'Invoice Info Received',
          data: msgStr
        });
        this.wsServer.sendEventsToAllLNClients(msgStr, selectedNode);
      })
      .catch((errRes) => {
        const err = this.common.handleError(
          errRes,
          'Invoices',
          'Subscribe to Invoice Error for ' + rHash,
          selectedNode
        );
        const errStr =
          typeof err === 'object' && err.message
            ? JSON.stringify({ error: err.message + ' ' + rHash })
            : typeof err === 'object'
            ? JSON.stringify({ error: err + ' ' + rHash })
            : '{ "error": ' + err + ' ' + rHash + ' }';
  ...
  public subscribeToPayment = (options: any, selectedNode: CommonSelectedNode, paymentHash: string) => {
    this.logger.log({
      ...
      level: 'INFO',
      fileName: 'WebSocketClient',
      msg: 'Subscribing to Payment ' + paymentHash + ' ..'
    });
    ...
    request(options)
      .then((msg) => {
        this.logger.log({
          ...
          level: 'INFO',
          fileName: 'WebSocketClient',
          msg: 'Payment Information Received for ' + paymentHash
        });
        msg['type'] = 'payment';
        msg['source'] = 'LND';
        const msgStr = JSON.stringify(msg);
      ...
      .catch((errRes) => {
        const err = this.common.handleError(
          errRes,
          'Payment',
          'Subscribe to Payment Error for ' + paymentHash,
          selectedNode
        );
        const errStr =
          typeof err === 'object' && err.message
            ? JSON.stringify({ error: err.message + ' ' + paymentHash })
            : typeof err === 'object'
            ? JSON.stringify({ error: err + ' ' + paymentHash })
            : '{ "error": ' + err + ' ' + paymentHash + ' }';
        this.wsServer.sendErrorToAllLNClients(errStr, selectedNode);
      });
  };
  ...
  public setOptionsForSelNode = (selectedNode: CommonSelectedNode) => {
    ...
    try {
      options['headers'] = {
        'Grpc-Metadata-macaroon': fs.readFileSync(join(selectedNode.macaroon_path, 'admin.macaroon')).toString('hex')
      };
    } catch (err) {
      this.logger.log({
        selectedNode: selectedNode,
  ...
  public disconnect = (selectedNode: CommonSelectedNode) => {
    ...
    if (clientExists) {
      this.logger.log({
        ...
        msg: "Disconnecting from the LND's Websocket Server.."
      });
  ...
  public updateSelectedNode = (newSelectedNode: CommonSelectedNode) => {
    ...
    if (
      this.webSocketClients[clientIdx].selectedNode.ln_version === '' ||
      !this.webSocketClients[clientIdx].selectedNode.ln_version ||
      this.common.isVersionCompatible(this.webSocketClients[clientIdx].selectedNode.ln_version, '0.11.0')
    ) {
