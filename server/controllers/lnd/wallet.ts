...
export const genSeed = (req, res, next) => {
  ...
  if (req.params.passphrase) {
    options.url =
      ...
      '/v1/genseed?aezeed_passphrase=' +
      Buffer.from(atob(req.params.passphrase)).toString('base64');
  } else {
...
export const operateWallet = (req, res, next) => {
  ...
  request(options)
    .then((body) => {
      ...
      if (!body) {
        const err = common.handleError(
          { statusCode: 500, message: 'Wallet Error', error: err_message },
          'Wallet',
          err_message,
        ...
        if (
          (body.code === 1 && body.error === 'context canceled') ||
          (body.code === 14 && body.error === 'transport is closing')
        ) {
          ...
          const errMsg =
            body.error && typeof body.error === 'object'
              ...
              : err_message;
          const err = common.handleError(
            { statusCode: 500, message: 'Wallet Error', error: errMsg },
            'Wallet',
            errMsg,
            req.session.selectedNode
          );
        ...
        logger.log({
          ...
          level: 'INFO',
          fileName: 'Wallet',
          msg: 'Wallet Unlocked/Initialized',
          data: body
        });
        res.status(201).json('Successful');
      }
    ...
    .catch((errRes) => {
      if (
        (errRes.error.code === 1 && errRes.error.error === 'context canceled') ||
        (errRes.error.code === 14 && errRes.error.error === 'transport is closing')
      ) {
        res.status(201).json('Successful');
      } else {
