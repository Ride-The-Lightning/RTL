        ...
        return Promise.all(
          ...
          .then((values) => {
            ...
            logger.log({
              ...
              level: 'INFO',
              fileName: 'Channels',
              msg: 'Sorted Channels List Received',
              data: body
            });
  ...
  request(options)
    .then((body) => {
      ...
      if (body.pending_open_channels && body.pending_open_channels.length > 0) {
        body.pending_open_channels.map((channel) =>
          promises.push(getAliasForChannel(req.session.selectedNode, channel.channel))
        );
      }
      if (body.pending_closing_channels && body.pending_closing_channels.length > 0) {
        body.pending_closing_channels.map((channel) =>
          promises.push(getAliasForChannel(req.session.selectedNode, channel.channel))
        );
      }
      if (body.pending_force_closing_channels && body.pending_force_closing_channels.length > 0) {
        body.pending_force_closing_channels.map((channel) =>
          promises.push(getAliasForChannel(req.session.selectedNode, channel.channel))
        );
      }
      if (body.waiting_close_channels && body.waiting_close_channels.length > 0) {
        body.waiting_close_channels.map((channel) =>
          promises.push(getAliasForChannel(req.session.selectedNode, channel.channel))
        );
      }
      return Promise.all(promises)
        .then((values) => {
          logger.log({
            ...
            level: 'INFO',
            fileName: 'Channels',
            msg: 'Pending Channels List Received',
            data: body
          });
          return res.status(200).json(body);
        })
        .catch((errRes) => {
          const err = common.handleError(
            errRes,
            'Channels',
            'Get Pending Channel Aliases Error',
            req.session.selectedNode
          );
          return res.status(err.statusCode).json({ message: err.message, error: err.error });
        });
        ...
        return Promise.all(
          ...
          .then((values) => {
            ...
            logger.log({
              ...
              level: 'INFO',
              fileName: 'Channels',
              msg: 'Closed Channels List Received',
              data: body
            });
            return res.status(200).json(body);
          })
          .catch((errRes) => {
            const err = common.handleError(
              errRes,
              'Channels',
              'Get Closed Channel Aliases Error',
              req.session.selectedNode
            );
            return res.status(err.statusCode).json({ message: err.message, error: err.error });
          });
...
export const closeChannel = (req, res, next) => {
  try {
    ...
    if (!req.session.selectedNode) {
      const err = common.handleError(
        { message: "Session Expired after a day's inactivity.", statusCode: 401 },
        'Session Expired',
    ...
    logger.log({
      ...
      level: 'DEBUG',
      fileName: 'Channels',
      msg: 'Closing Channel Options URL',
      data: options.url
    });
