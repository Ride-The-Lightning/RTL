...
export const decodePayments = (req, res, next) => {
  ...
  if (req.body.payments) {
    ...
    logger.log({
      ...
      level: 'INFO',
      fileName: 'PayRequest',
      msg: 'Empty Payment List Decoded'
    });
...
export const getPayments = (req, res, next) => {
  ...
  options.url =
    ...
    '/v1/payments?max_payments=' +
    req.query.max_payments +
    '&index_offset=' +
    req.query.index_offset +
    '&reversed=' +
    req.query.reversed;
  request(options)
    .then((body) => {
...
export const getAllLightningTransactions = (req, res, next) => {
  logger.log({
    ...
    level: 'INFO',
    fileName: 'Payments',
    msg: 'Getting All Lightning Transactions..'
  });
  ...
  options2.url =
    req.session.selectedNode.ln_server_url + '/v1/invoices?num_max_invoices=100000&index_offset=0&reversed=true';
  logger.log({
    selectedNode: req.session.selectedNode,
    level: 'DEBUG',
    fileName: 'Payments',
    msg: 'All Payments Options',
    data: options1
  });
  logger.log({
    selectedNode: req.session.selectedNode,
    level: 'DEBUG',
    fileName: 'Payments',
    msg: 'All Invoices Options',
    data: options2
  });
  // return Promise.all([request(options1), request(options2)]).then((values) => {
  return Promise.all([{ payments: [] }, request(options2)])
    .then((values) => {
      logger.log({
        selectedNode: req.session.selectedNode,
        level: 'INFO',
        fileName: 'Payments',
        msg: 'All Lightning Transactions Received',
        data: { totalPayments: values[0].payments.length || 0, totalInvoices: values[1].invoices.length || 0 }
      });
  ...
  request(options)
    .then((body) => {
      logger.log({
        ...
        level: 'INFO',
        fileName: 'Payments',
        msg: 'Payment Information Received for ' + req.params.paymentHash,
        data: body
      });
