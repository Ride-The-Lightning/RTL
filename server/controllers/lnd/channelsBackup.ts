    ...
    if (exists) {
      ...
      if (verify_backup !== '') {
        ...
        const err = common.handleError(
          { statusCode: 404, message: 'Verify Channel Error', error: errMsg },
          'ChannelBackup',
          errMsg,
  ...
  if (req.params.channelPoint === 'ALL') {
    ...
    if (exists) {
      ...
      if (restore_backup !== '') {
        ...
        const err = common.handleError(
          { statusCode: 404, message: 'Restore Channel Error', error: errMsg },
          'ChannelBackup',
          errMsg,
          req.session.selectedNode
        );
        ...
        fs.rename(channel_restore_file, channel_restore_file + '.restored', () => {
          getFilesList(req.session.selectedNode.channel_backup_path, (getFilesListRes) => {
            if (getFilesListRes.error) {
              ...
              const err = common.handleError(
                { statusCode: 500, message: 'Restore Channel Error', error: errMsg },
                'ChannelBackup',
                errMsg,
                req.session.selectedNode
              );
