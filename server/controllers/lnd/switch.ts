...
export const getAllForwardingEvents = (req, start, end, offset, callback) => {
  ...
  logger.log({ selectedNode: req.session.selectedNode, level: 'INFO', fileName: 'Switch', msg: 'Forwarding History Params', data: options.form });
  return request.post(options).then((body) => {
    logger.log({ selectedNode: req.session.selectedNode, level: 'INFO', fileName: 'Switch', msg: 'Forwarding Events', data: body });
