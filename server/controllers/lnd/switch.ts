...
const responseData = {
  switch: { forwarding_events: [], last_offset_index: 0 },
  fees: { forwarding_events: [], last_offset_index: 0 }
};
  ...
  return request
    ...
    .then((body) => {
      logger.log({
        ...
        level: 'INFO',
        fileName: 'Switch',
        msg: 'Forwarding Events Received',
        data: body
      });
      ...
      if (!body.last_offset_index || body.last_offset_index < offset + num_max_events) {
        ...
        if (responseData[caller].forwarding_events) {
          responseData[caller].forwarding_events = common.sortDescByKey(
            responseData[caller].forwarding_events,
            'timestamp'
          );
        }
