...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  public apiCallStatusBlockchainBalance$: Observable<ApiCallStatusPayload>;
  public apiCallStatusChannels$: Observable<ApiCallStatusPayload>;
  public apiCallStatusPendingChannels$: Observable<ApiCallStatusPayload>;
  ...
  ngOnInit() {
    const nodeInfoSelector$ = this.store.select(nodeInfoAndAPIStatus).pipe(
      takeUntil(this.unSubs[0]),
      withLatestFrom(this.store.select(rootSelectedNode)),
      shareReplay(1)
    );

    nodeInfoSelector$.subscribe(([infoStatusSelector, nodeSettings]: [{ information: GetInfo | null, apiCallStatus: ApiCallStatusPayload }, nodeSettings: Node]) => {
      this.errorMessages[0] = '';
      if (infoStatusSelector.apiCallStatus.status === APICallStatusEnum.ERROR) {
        this.errorMessages[0] = (typeof (infoStatusSelector.apiCallStatus.message) === 'object') ? JSON.stringify(infoStatusSelector.apiCallStatus.message) : infoStatusSelector.apiCallStatus.message ? infoStatusSelector.apiCallStatus.message : '';
      }
    ...
    this.apiCallStatusNodeInfo$ = nodeInfoSelector$.pipe(
      map(([infoStatusSelector]) => infoStatusSelector.apiCallStatus)
    );
    ...
    feesSelector$.subscribe((feesSelector: { fees: Fees; apiCallStatus: ApiCallStatusPayload }) => {
      ...
      if (feesSelector.apiCallStatus?.status === APICallStatusEnum.ERROR) {
        this.errorMessages[1] = typeof feesSelector.apiCallStatus.message === 'object' ? JSON.stringify(feesSelector.apiCallStatus.message) : feesSelector.apiCallStatus.message || '';
      }
    ...
    const bcBalanceSelector$ = this.store.select(blockchainBalance).pipe(takeUntil(this.unSubs[2]), shareReplay(1));

    bcBalanceSelector$.subscribe((bcBalanceSelector: { blockchainBalance: BlockchainBalance, apiCallStatus: ApiCallStatusPayload }) => {
      this.errorMessages[2] = '';
      if (bcBalanceSelector.apiCallStatus.status === APICallStatusEnum.ERROR) {
        this.errorMessages[2] = (typeof (bcBalanceSelector.apiCallStatus.message) === 'object') ? JSON.stringify(bcBalanceSelector.apiCallStatus.message) : bcBalanceSelector.apiCallStatus.message ? bcBalanceSelector.apiCallStatus.message : '';
      }
    ...
    this.apiCallStatusBlockchainBalance$ = bcBalanceSelector$.pipe(map((bcs) => bcs.apiCallStatus));

    const pendingChannelsSelector$ = this.store.select(pendingChannels).pipe(takeUntil(this.unSubs[3]), shareReplay(1));

    pendingChannelsSelector$.subscribe((pendingChannelsSelector: { pendingChannels: PendingChannels, pendingChannelsSummary: PendingChannelsSummary, apiCallStatus: ApiCallStatusPayload }) => {
      this.errorMessages[4] = '';
      if (pendingChannelsSelector.apiCallStatus.status === APICallStatusEnum.ERROR) {
        this.errorMessages[4] = (typeof (pendingChannelsSelector.apiCallStatus.message) === 'object') ? JSON.stringify(pendingChannelsSelector.apiCallStatus.message) : pendingChannelsSelector.apiCallStatus.message ? pendingChannelsSelector.apiCallStatus.message : '';
      }
    ...
    this.apiCallStatusPendingChannels$ = pendingChannelsSelector$.pipe(map((pcs) => pcs.apiCallStatus));

    const channelsSelector$ = this.store.select(channels).pipe(takeUntil(this.unSubs[4]), shareReplay(1));
    ...
    channelsSelector$.subscribe((channelsSelector: { channels: Channel[], channelsSummary: ChannelsSummary, lightningBalance: LightningBalance, apiCallStatus: ApiCallStatusPayload }) => {
      ...
      if (channelsSelector.apiCallStatus.status === APICallStatusEnum.ERROR) {
        this.errorMessages[3] = (typeof (channelsSelector.apiCallStatus.message) === 'object') ? JSON.stringify(channelsSelector.apiCallStatus.message) : channelsSelector.apiCallStatus.message ? channelsSelector.apiCallStatus.message : '';
      }
