...
connect.validateNodeConfig = (config) => {
  ...
  if (config.nodes && config.nodes.length > 0) {
    config.nodes.forEach((node, idx) => {
      ...
      common.nodes[idx].channel_backup_path = process.env.CHANNEL_BACKUP_PATH ? process.env.CHANNEL_BACKUP_PATH : (node.Settings.channelBackupPath) ? node.Settings.channelBackupPath : common.rtl_conf_file_path + common.path_separator + 'backend' + common.path_separator + 'channels-backup' + common.path_separator + 'node-' + node.index;
      try {
      ...
      common.nodes[idx].log_file = common.rtl_conf_file_path + '/backend/logs/RTL-Node-' + node.index + '.log';
