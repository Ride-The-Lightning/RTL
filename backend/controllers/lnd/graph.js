...
export const getGraphInfo = (req, res, next) => {
    ...
    request
        ...
        logger.log({
            ...
            level: 'INFO',
            fileName: 'Graph',
            msg: 'Graph Information Received',
            data: body
        });
        res.status(200).json(body);
    })
...
export const getGraphNode = (req, res, next) => {
    logger.log({
        ...
        level: 'INFO',
        fileName: 'Graph',
        msg: 'Getting Graph Node Information..'
    });
    options = common.getOptions(req);
    if (options.error) {
        return res.status(options.statusCode).json({ message: options.message, error: options.error });
    ...
    request(options)
        ...
        logger.log({
            ...
            level: 'INFO',
            fileName: 'Graph',
            msg: 'Graph Node Information Received',
            data: body
        });
        res.status(200).json(body);
    })
...
export const getGraphEdge = (req, res, next) => {
    logger.log({
        ...
        level: 'INFO',
        fileName: 'Graph',
        msg: 'Getting Graph Edge Information..'
    });
    options = common.getOptions(req);
    if (options.error) {
        return res.status(options.statusCode).json({ message: options.message, error: options.error });
    ...
    request(options)
        ...
        logger.log({
            ...
            level: 'INFO',
            fileName: 'Graph',
            msg: 'Graph Edge Information Received',
            data: body
        });
        res.status(200).json(body);
    })
...
export const getQueryRoutes = (req, res, next) => {
    ...
    options.url =
        req.session.selectedNode.ln_server_url + '/v1/graph/routes/' + req.params.destPubkey + '/' + req.params.amount;
    ...
    request(options)
        ...
        if (body.routes &&
            ...
            body.routes[0].hops &&
            body.routes[0].hops.length &&
            body.routes[0].hops.length > 0) {
            return Promise.all(body.routes[0].hops.map((hop) => getAliasFromPubkey(req.session.selectedNode, hop.pub_key)))
                ...
                logger.log({
                    ...
                    level: 'INFO',
                    fileName: 'Graph',
                    msg: 'Graph Routes with Alias Received',
                    data: body
                });
                res.status(200).json(body);
            })
...
export const getRemoteFeePolicy = (req, res, next) => {
    ...
    request(options)
        ...
        logger.log({
            ...
            level: 'INFO',
            fileName: 'Graph',
            msg: 'Remote Fee Policy Received',
            data: remoteNodeFee
        });
