...
export const getAliasFromPubkey = (selNode, pubkey) => {
    ...
    return request(options).then((res) => {
        logger.log({ selectedNode: selNode, level: 'INFO', fileName: 'Graph', msg: 'Alias', data: res.node.alias });
...
export const getDescribeGraph = (req, res, next) => {
    ...
    request.get(options).then((body) => {
        logger.log({ selectedNode: req.session.selectedNode, level: 'INFO', fileName: 'Graph', msg: 'Network Graph', data: body });
...
export const getGraphInfo = (req, res, next) => {
    ...
    request.get(options).then((body) => {
        logger.log({ selectedNode: req.session.selectedNode, level: 'INFO', fileName: 'Graph', msg: 'Graph Information', data: body });
        res.status(200).json(body);
    }).catch((errRes) => {
...
export const getGraphNode = (req, res, next) => {
    ...
    request(options).then((body) => {
        logger.log({ selectedNode: req.session.selectedNode, level: 'INFO', fileName: 'Graph', msg: 'Graph Node Information', data: body });
        res.status(200).json(body);
    }).catch((errRes) => {
...
export const getGraphEdge = (req, res, next) => {
    ...
    request(options).then((body) => {
        logger.log({ selectedNode: req.session.selectedNode, level: 'INFO', fileName: 'Graph', msg: 'Graph Edge Information', data: body });
        res.status(200).json(body);
    }).catch((errRes) => {
...
export const getQueryRoutes = (req, res, next) => {
    ...
    logger.log({ selectedNode: req.session.selectedNode, level: 'INFO', fileName: 'Graph', msg: 'Query Routes URL', data: options.url });
    request(options).then((body) => {
        logger.log({ selectedNode: req.session.selectedNode, level: 'INFO', fileName: 'Graph', msg: 'Query Routes', data: body });
        if (body.routes && body.routes.length && body.routes.length > 0 && body.routes[0].hops && body.routes[0].hops.length && body.routes[0].hops.length > 0) {
            return Promise.all(body.routes[0].hops.map((hop) => getAliasFromPubkey(req.session.selectedNode, hop.pub_key))).
                ...
                logger.log({ selectedNode: req.session.selectedNode, level: 'INFO', fileName: 'Graph', msg: 'Graph Routes', data: body });
                res.status(200).json(body);
            }).
                catch((errRes) => {
...
export const getRemoteFeePolicy = (req, res, next) => {
    ...
    options.url = req.session.selectedNode.ln_server_url + '/v1/graph/edge/' + req.params.chanid;
    request(options).then((body) => {
        logger.log({ selectedNode: req.session.selectedNode, level: 'INFO', fileName: 'Graph', msg: 'Edge Info', data: body });
        let remoteNodeFee = {};
