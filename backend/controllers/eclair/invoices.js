        ...
        return Promise.all([request(options1), request(options2)]).
            ...
            if (invoices && invoices.length > 0) {
                return Promise.all(invoices.map((invoice) => getReceivedPaymentInfo(req.session.selectedNode.ln_server_url, invoice))).
                    then((values) => {
                    body = common.sortDescByKey(invoices, 'expiresAt');
                    logger.log({ selectedNode: req.session.selectedNode, level: 'INFO', fileName: 'Invoices', msg: 'List Invoices Received', data: invoices });
                    return res.status(200).json(invoices);
                }).
                    catch((errRes) => {
...
export const createInvoice = (req, res, next) => {
    ...
    request.post(options).then((body) => {
        logger.log({ selectedNode: req.session.selectedNode, level: 'INFO', fileName: 'Invoice', msg: 'Create Invoice', data: body });
        if (body.amount) {
