...
export const countInvoices = (baseOptions, lnServerUrl, from, to) => {
    const hasInvoiceAt = (skip) => fetchInvoicePage(baseOptions, lnServerUrl, from, to, 1, skip).then((page) => page.length > 0);
    // `present` is an offset known to hold an invoice, `absent` one known to be past the end.
    const bisect = (present, absent) => {
        if (absent - present <= 1) {
    ...
    const gallop = (present, absent) => hasInvoiceAt(absent).then((found) => (found ? gallop(absent, absent * 2) : bisect(present, absent)));
...
export const listInvoices = (req, res, next) => {
    ...
    if (common.read_dummy_data) {
        return common.getDummyData('Invoices', req.session.selectedNode.lnImplementation).then(([invoices]) => Promise.all(invoices?.map((invoice) => getReceivedPaymentInfo(baseOptions, lnServerUrl, invoice))).
