...
export const simplifyAllChannels = (selNode, channels) => {
    ...
    channels.forEach((channel) => {
        ...
        simplifiedChannels.push({
            ...
            channelFlags: channel.data && channel.data.commitments && channel.data.commitments.channelFlags
                ? channel.data.commitments.channelFlags
                : 0,
            toLocal: channel.data.commitments.localCommit.spec.toLocal
                ? Math.round(+channel.data.commitments.localCommit.spec.toLocal / 1000)
                : 0,
            toRemote: channel.data.commitments.localCommit.spec.toRemote
                ? Math.round(+channel.data.commitments.localCommit.spec.toRemote / 1000)
                : 0,
            shortChannelId: channel.data && channel.data.shortChannelId ? channel.data.shortChannelId : '',
            isFunder: channel.data &&
                channel.data.commitments &&
                channel.data.commitments.localParams &&
                channel.data.commitments.localParams.isFunder
                ? channel.data.commitments.localParams.isFunder
                : false,
            buried: channel.data && channel.data.buried ? channel.data.buried : false,
            feeBaseMsat: channel.data && channel.data.channelUpdate && channel.data.channelUpdate.feeBaseMsat
                ? channel.data.channelUpdate.feeBaseMsat
                : 0,
            feeRatePerKw: channel.data.commitments.localCommit.spec.feeratePerKw
                ? channel.data.commitments.localCommit.spec.feeratePerKw
                : 0,
            feeProportionalMillionths: channel.data && channel.data.channelUpdate && channel.data.channelUpdate.feeProportionalMillionths
...
export const getChannels = (req, res, next) => {
    ...
    if (common.read_dummy_data) {
        common.getDummyData('Channels', req.session.selectedNode.ln_implementation).then((data) => {
            ...
            if (body && body.length) {
                return simplifyAllChannels(req.session.selectedNode, body).then((simplifiedChannels) => {
                    logger.log({
                        ...
                        level: 'INFO',
                        fileName: 'Channels',
                        msg: 'Simplified Channels with Alias Received',
                        data: simplifiedChannels
                    });
                ...
                logger.log({
                    ...
                    level: 'INFO',
                    fileName: 'Channels',
                    msg: 'Empty Channels List Received'
                });
                res.status(200).json({
                    activeChannels: [],
                    pendingChannels: [],
                    inactiveChannels: [],
                    lightningBalances: { localBalance: 0, remoteBalance: 0 },
                    channelStatus: {
...
export const updateChannelRelayFee = (req, res, next) => {
    logger.log({
        ...
        level: 'INFO',
        fileName: 'Channels',
        msg: 'Updating Channel Relay Fee..'
    });
    options = common.getOptions(req);
    if (options.error) {
        return res.status(options.statusCode).json({ message: options.message, error: options.error });
