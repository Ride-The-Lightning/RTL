...
export const verifyToken = (twoFAToken) => !!(common.rtl_secret2fa &&
    ...
    if (+common.rtl_sso) {
        if (req.body.authenticateWith === 'JWT' && jwt.verify(req.body.authenticationValue, common.secret_key)) {
            ...
            if (common.cookie_value.trim().length >= 32 &&
                crypto.timingSafeEqual(Buffer.from(crypto.createHash('sha256').update(common.cookie_value).digest('hex'), 'utf-8'), Buffer.from(req.body.authenticationValue, 'utf-8'))) {
