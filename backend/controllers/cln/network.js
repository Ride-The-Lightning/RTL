...
export const getRoute = (req, res, next) => {
    ...
    options.url =
        req.session.selectedNode.ln_server_url + '/v1/network/getRoute/' + req.params.destPubkey + '/' + req.params.amount;
    request(options)
        .then((body) => {
        logger.log({
            selectedNode: req.session.selectedNode,
...
export const feeRates = (req, res, next) => {
    ...
    request(options)
        ...
        logger.log({
            ...
            level: 'INFO',
            fileName: 'Network',
            msg: 'Network Fee Rates Received for ' + req.params.feeRateStyle,
            data: body
        });
        res.status(200).json(body);
    })
