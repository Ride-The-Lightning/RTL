...
const resolvedHost = hostUndefined ? 'global' : common.host; //'127.0.0.1' <-- this will become new default soon. swap with 'global' for breaking change
const resolvedPort = common.port;
const tcpModule = common.ssl ? https : http;
const fullUrl = `${common.ssl ? 'https' : 'http'}://${hostUndefined ? 'localhost' : resolvedHost}:${resolvedPort}`;
